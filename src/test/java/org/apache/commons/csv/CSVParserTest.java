 ...
 * fails, you should follow a top-down approach for fixing a potential bug (its
 * likely that the parser itself fails if the lexer has problems...).
 *
 * @version $Id: CSVParserTest.java 1715712 2015-11-22 22:03:06Z ggregory $
 */
public class CSVParserTest {
	...
	public void testBackslashEscapingOld() throws IOException {
		...
		final String[][] res = { { "one", "two", "three" }, { "on\\\"e", "two" },
		        { "on\"e", "two" }, { "one", "tw\"o" }, { "one", "t\\,wo" }, // backslash
		                                                                     // in
		                                                                     // quotes
		                                                                     // only
		                                                                     // escapes
		...
		for (int i = 0; i < res.length; i++)
			assertArrayEquals(res[i], records.get(i)
			                                 .values());
		parser.close();
	}
	...
	public void testBOM() throws IOException {
		final URL url = ClassLoader.getSystemClassLoader()
		                           .getResource("CSVFileParser/bom.csv");
		final CSVParser parser = CSVParser.parse(url, Charset.forName("UTF-8"),
	...
	public void testClose() throws Exception {
		...
		final CSVParser parser = CSVFormat.DEFAULT.withCommentMarker('#')
		                                          .withHeader()
		                                          .parse(in);
	...
	public void testGetHeaderMap() throws Exception {
		...
		final Iterator<String> columnNames = headerMap.keySet()
		                                              .iterator();
	...
	public void testGetLine() throws IOException {
		...
		for (final String[] re : RESULT)
			assertArrayEquals(re, parser.nextRecord()
			                            .values());
	...
	public void testGetRecords() throws IOException {
		...
		for (int i = 0; i < RESULT.length; i++)
			assertArrayEquals(RESULT[i], records.get(i)
			                                    .values());
		parser.close();
	}
	...
	@Test
	public void testWeirdHeader() throws Exception {
		final Reader in = new StringReader("Site ID: HWHB0643M\nSerial Number: 06400084\nFirmware Version: 3.23\na,b,c\n1,2,3\nx,y,z");

		final Iterator<CSVRecord> records = CSVFormat.DEFAULT.withHeader("a", "b", "c")
		                                                     .withSkipHeaderRecord()
		                                                     .withSkipHeaderRecord(3)
		                                                     .parse(in)
		                                                     .iterator();
	...
	public void testHeaderComment() throws Exception {
		...
		final Iterator<CSVRecord> records = CSVFormat.DEFAULT.withCommentMarker('#')
		                                                     .withHeader()
		                                                     .parse(in)
		                                                     .iterator();
	...
	public void testIterator() throws Exception {
		...
		final Iterator<CSVRecord> iterator = CSVFormat.DEFAULT.parse(in)
		                                                      .iterator();
		...
		assertArrayEquals(new String[] { "a", "b", "c" }, iterator.next()
		                                                          .values());
		assertArrayEquals(new String[] { "1", "2", "3" }, iterator.next()
		                                                          .values());
		assertTrue(iterator.hasNext());
		assertTrue(iterator.hasNext());
		assertTrue(iterator.hasNext());
		assertArrayEquals(new String[] { "x", "y", "z" }, iterator.next()
		                                                          .values());
	...
	@Test
	// TODO this may lead to strange behavior, throw an exception if iterator()
	...
	public void testParserUrlNullCharsetFormat() throws Exception {
		final CSVParser parser = CSVParser.parse(new URL("http://commons.apache.org"), null,
		                                         CSVFormat.DEFAULT);
		parser.close();
	}
	...
	public void testParseUrlCharsetNullFormat() throws Exception {
		final CSVParser parser = CSVParser.parse(new URL("http://commons.apache.org"),
	...
	public void testSkipSetAltHeaders() throws Exception {
		final Reader in = new StringReader("a,b,c\n1,2,3\nx,y,z");
		final Iterator<CSVRecord> records = CSVFormat.DEFAULT.withHeader("X", "Y", "Z")
		                                                     .withSkipHeaderRecord()
		                                                     .parse(in)
		                                                     .iterator();
		final CSVRecord record = records.next();
	...
	private void validateLineNumbers(final String lineSeparator) throws IOException {
		final CSVParser parser = CSVParser.parse("a" + lineSeparator + "b" + lineSeparator
		        + "c", CSVFormat.DEFAULT.withRecordSeparator(lineSeparator));
		assertEquals(0, parser.getCurrentLineNumber());
	...
	private void validateRecordPosition(final String lineSeparator) throws IOException {
		final String nl = lineSeparator; // used as linebreak in values for
		                                 // better distinction
		...
		parser = new CSVParser(new StringReader(code.substring((int) positionRecord3)), format,
		                       positionRecord3, 3);
	...
	public void testIgnoreCaseHeaderMapping() throws Exception {
		...
		final Iterator<CSVRecord> records = CSVFormat.DEFAULT.withHeader("One", "TWO", "three")
